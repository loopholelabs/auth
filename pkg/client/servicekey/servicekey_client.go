...
func (a *Client) PostServicekeyLogin(params *PostServicekeyLoginParams, opts ...ClientOption) (*PostServicekeyLoginOK, error) {
	...
	if params == nil {
		params = NewPostServicekeyLoginParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "PostServicekeyLogin",
		Method:             "POST",
		PathPattern:        "/servicekey/login",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &PostServicekeyLoginReader{formats: a.formats},
	...
	success, ok := result.(*PostServicekeyLoginOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for PostServicekeyLogin: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
