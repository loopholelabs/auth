...
func (a *Google) GoogleLogin(ctx *fiber.Ctx) error {
	...
	redirect, err := a.options.Google().StartFlow(ctx.Context(), ctx.Query("next", a.options.NextURL()), ctx.Query("organization"), identifier)
	if err != nil {
...
func (a *Google) GoogleCallback(ctx *fiber.Ctx) error {
	...
	userID, organization, nextURL, deviceIdentifier, err := a.options.Google().CompleteFlow(ctx.Context(), code, state)
	if err != nil {
	...
	cookie, err := a.options.Manager().CreateSession(ctx, sessionKind, a.options.Google().Key(), userID, organization)
	if cookie == nil {
		return err
	}
