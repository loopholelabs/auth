...
func (d *Magic) MagicFlow(ctx *fiber.Ctx) error {
	...
	if identifier != "" {
		...
		exists, err := d.options.Device().FlowExists(ctx.Context(), identifier)
		if err != nil {
	...
	secret, err := d.options.Magic().StartFlow(ctx.Context(), email, ctx.IP(), ctx.Query("next", d.options.NextURL()), ctx.Query("organization"), identifier)
	if err != nil {
	...
	encrypted, err := d.options.Manager().EncryptMagic(email, secret)
	if err != nil {
...
func (d *Magic) MagicCallback(ctx *fiber.Ctx) error {
	...
	email, secret, err := d.options.Manager().DecryptMagic(token)
	if err != nil {
	...
	organization, nextURL, deviceIdentifier, err := d.options.Magic().CompleteFlow(ctx.Context(), email, secret)
	if err != nil {
	...
	if deviceIdentifier != "" {
		err = d.options.Device().CompleteFlow(ctx.Context(), deviceIdentifier, cookie.Value, cookie.Expires)
		if err != nil {
