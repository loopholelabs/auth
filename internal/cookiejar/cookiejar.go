...
func defaultPath(path string) string {
	if len(path) == 0 || path[0] != '/' {
		return "/" // Source is empty or malformed.
	}

	i := strings.LastIndex(path, "/") // Source starts with "/", so i != -1.
	if i == 0 {
		return "/" // Source has the form "/abc".
	}
	return path[:i] // Source is either of form "/abc/xyz" or "/abc/xyz/".
}
